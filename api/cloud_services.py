...
def get_storage_client(provider: Optional[str] = None):
    ...
    if not client:
        raise HTTPException(
            status_code=503,
            detail=f"Storage service not available for provider: " f"{config.get_provider_name()}",
        )
...
def get_database_client(provider: Optional[str] = None):
    ...
    if not client:
        raise HTTPException(
            status_code=503,
            detail=f"Database service not available for provider: " f"{config.get_provider_name()}",
        )
    return client, config.get_provider_name()
...
def get_ai_client(provider: Optional[str] = None):
    ...
    if not client:
        raise HTTPException(
            status_code=503,
            detail=f"AI service not available for provider: " f"{config.get_provider_name()}",
        )
    return client, config.get_provider_name()
...
async def get_providers_info():
    ...
    return CloudProvidersResponse(current_provider=current_provider, providers=providers)
...
async def upload_file(
    ...
    try:
        ...
        result = client.upload_file(
            temp_file_path, path, public=make_public, content_type=file.content_type
        )
...
async def get_file_url(path: str, expiration: int = 3600, provider: Optional[str] = None):
...
async def list_tables(provider: Optional[str] = None):
    ...
    return DatabaseListResponse(
        success=True, tables=tables, count=len(tables), provider=provider_name
    )


@router.post("/database/tables/{table_name}")
async def create_table(table_name: str, key_schema: Dict[str, Any], provider: Optional[str] = None):
...
async def query_items(request: QueryRequest, provider: Optional[str] = None):
    ...
    items = client.query_items(request.table_name, request.query, limit=request.limit)
...
async def list_models(provider: Optional[str] = None):
    ...
    return AIModelsResponse(success=True, models=models, count=len(models), provider=provider_name)


@router.post("/ai/predict/{model_name}")
async def predict(model_name: str, data: Dict[str, Any], provider: Optional[str] = None):
