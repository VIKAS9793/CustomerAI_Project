...
from src.utils.security import (
    create_jwt_token, validate_jwt_token, get_password_hash,
    authentication, rate_limiter, DeviceSecurityValidator
)
from src.utils.error_handler import CustomException, error_handler
...
app.add_middleware(
    TrustedHostMiddleware, allowed_hosts=[os.getenv("ALLOWED_HOST", "*")]
)
...
async def login_for_access_token(form_data: OAuth2PasswordRequestForm = Depends()):
    ...
    # Check device integrity if device headers are provided
    device_headers = {}
    for header in ["X-Device-Id", "X-Device-Model", "X-OS-Version", "X-App-Version"]:
        if header in request.headers:
            device_headers[header.replace("X-", "").lower()] = request.headers.get(header)
    
    if device_headers:
        security_result = device_security.validate_device(device_headers)
        if not security_result["secure"]:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail=f"Device security check failed: {security_result['reason']}",
                headers={"WWW-Authenticate": "Bearer"},
            )
...
@app.middleware("http")
async def validate_device_security(request: Request, call_next):
    ...
    if not is_secure:
        return JSONResponse(
            status_code=status.HTTP_403_FORBIDDEN,
            content={
                "detail": "Security requirements not met",
                "reason": reason
            }
        )
