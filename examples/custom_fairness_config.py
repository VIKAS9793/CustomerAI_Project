...
from src.config.fairness_config import FairnessConfig
...
def example_financial_services_config():
    ...
    print(
        "Financial services configuration saved to examples/configs/financial_services_config.json"
    )
    return config
...
def example_healthcare_config():
    ...
    config = {
        ...
        "significance": {"pvalue_threshold": 0.05, "confidence_interval": 0.95},
        "reporting": {
            "severity_levels": {"high": 0.15, "medium": 0.08, "low": 0.03},
            "include_recommendations": True,
        },
        "mitigation": {
            ...
            "strategy_params": {
                "reweighing": {"weight_bound": 5.0},  # Limit weight adjustments for healthcare data
                "calibrated_equalized_odds": {
...
def run_with_config(config, dataset, name):
    ...
    results = bias_detector.detect_outcome_bias(
        dataset, attributes=["gender", "age_group"], outcome_columns=["loan_approved"]
    )
                ...
                if "outcome_summary" in attr_summary:
                    for outcome, outcome_summary in attr_summary["outcome_summary"].items():
                        ...
                        if "metrics_summary" in outcome_summary:
                            for metric, metric_summary in outcome_summary[
                                "metrics_summary"
                            ].items():
                                print(
                                    f"      {metric}: {metric_summary['value']:.4f} (threshold: {metric_summary['threshold']:.4f})"
                                )
...
def main():
    ...
    print(
        "Default configuration detected bias:",
        default_results["summary"].get("bias_detected", "N/A"),
    )
    print(
        "Financial services configuration detected bias:",
        financial_results["summary"].get("bias_detected", "N/A"),
    )
    print(
        "Healthcare configuration detected bias:",
        healthcare_results["summary"].get("bias_detected", "N/A"),
    )
