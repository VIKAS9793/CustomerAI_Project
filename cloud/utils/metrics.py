                            ...
                            if numeric_values:
                                ...
                                if len(numeric_values) >= 10:
                                    ...
                                    stats.update({
                                        ...
                                        "p99": sorted_values[int(length * 0.99)] if length >= 100 else None,
                                    })
                    ...
                    for o in operations:
                        ...
                        if metric_type_value:
                            if metric_type_value in operations[o]:
                                result[p][s][o][metric_type_value] = operations[o][metric_type_value]
                        else:
                        ...
                        for m in metrics[p][s][o]:
                            ...
                            for stat_name, stat_value in stats.items():
                                if stat_value is not None:
                                    if stat_name in ["mean", "p50", "p90", "p95", "p99"] and m == MetricType.LATENCY.value:
                                        # Format latency as milliseconds
                                        report.append(f"        {stat_name}: {stat_value * 1000:.2f} ms")
                                    elif stat_name in ["mean", "p50", "p90", "p95", "p99"] and m == MetricType.ERROR_RATE.value:
                                        # Format error rate as percentage
                                        report.append(f"        {stat_name}: {stat_value * 100:.2f}%")
                                    else:
    ...
    def decorator(func: Callable[..., T]) -> Callable[..., T]:
        ...
        def wrapper(*args, **kwargs) -> T:
            ...
            try:
                ...
                # Record success rate
                _metrics_manager.record_metric(
                    provider, service, op_name, MetricType.SUCCESS_RATE, 
