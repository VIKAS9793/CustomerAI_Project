...
from cloud.errors import CloudServiceUnavailableError, CloudTimeoutError
        ...
        with self.lock:
            ...
            if conn_id in self.in_use:
                ...
                if self.status != PoolStatus.CLOSED and pooled_conn.is_valid():
                    ...
                    current_time = time.time()
                    if (
                        current_time - pooled_conn.created_at > self.max_age
                        or self.max_uses > 0
                        and pooled_conn.use_count >= self.max_uses
                    ):
                        pooled_conn.close()
                        self.size -= 1
                    else:
...
class PoolManager:
    ...
    def get_pool(
        ...
        **pool_kwargs,
        ...
        with self.lock:
            ...
            if factory is None:
                raise ValueError(
                    f"Factory must be provided to create a new pool for {provider_str}/{service}/{name}"
                )
    ...
    def close_pool(
        self, provider: Union[CloudProvider, str], service: str, name: str = "default"
    ) -> None:
        """
...
def connection_pooled(
    provider: Union[CloudProvider, str], service: str, pool_name: str = "default"
