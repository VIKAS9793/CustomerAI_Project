...
import threading
import functools
from typing import Dict, Any, Optional, List, Union, Callable, TypeVar, Set, Tuple
from enum import Enum
import json

from cloud.config import CloudProvider
from cloud.errors import CloudServiceUnavailableError, CloudNetworkError, CloudTimeoutError
...
class ChaosTestingEngine:
        ...
        with self.lock:
            ...
            for injection_id, injection in self.injections.items():
                ...
                if scope == InjectionScope.ALL:
                    ...
                    match = (
                        injection["target"].lower() == provider.lower() and
                        injection["target_secondary"].lower() == service.lower()
                    )
                elif scope == InjectionScope.OPERATION:
                    # Match provider, service, and operation
                    match = (
                        injection["target"].lower() == provider.lower() and
                        injection["target_secondary"].lower() == service.lower() and
                        injection["parameters"].get("operation", "").lower() == operation.lower()
                    )
                else:
                    match = False
    ...
    def _inject_failure(
        ...
        if failure_mode == FailureMode.LATENCY:
            ...
            error_message = parameters.get("error_message", f"Chaos testing injected failure for {provider}/{service}/{operation}")
            ...
            if random.random() < parameters.get("throttle_probability", 0.5):
                error_message = parameters.get("error_message", f"Chaos testing injected throttling for {provider}/{service}/{operation}")
                raise CloudServiceUnavailableError(error_message)
            ...
            # Raise timeout error
            error_message = parameters.get("error_message", f"Chaos testing injected timeout for {provider}/{service}/{operation}")
                ...
                with self.lock:
                    if experiment_id in self.experiments:
                        ...
                        for inj in experiment["injections"]:
                            if inj["delayed"] and time.time() < experiment["created_at"] + inj["delay"]:
                        ...
                        if all_injections_added:
                            ...
                            logger.warning(f"Chaos experiment {experiment_id} completed before all injections were added")
            
            # Start cleanup thread
            threading.Thread(
                target=cleanup_experiment,
                daemon=True
            ).start()
            ...
            for injection_info in experiment["injections"]:
                ...
                if injection:
                    ...
                    injection_data.append({
                        "id": injection_id,
                        "failure_mode": str(injection["failure_mode"].value) if "failure_mode" in injection else "unknown",
                        "hit_count": injection.get("hit_count", 0),
                        "trigger_count": injection.get("trigger_count", 0),
                        "trigger_rate": injection.get("trigger_count", 0) / max(1, injection.get("hit_count", 0))
                    })
                else:
                    # Injection completed or never started
                    injection_data.append({
                        "id": injection_id,
                        "status": "completed" if injection_info.get("delayed", False) else "missing"
                    })
