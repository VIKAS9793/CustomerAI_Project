...
import functools
import time
from typing import Dict, Any, Optional, Callable, TypeVar, Union, Tuple, cast
...
from cloud.errors import CloudConfigurationError
...
class EncryptionManager:
    ...
    def __init__(self):
        ...
        self.key_pairs: Dict[str, Dict[str, bytes]] = {}  # {key_name: {"private": ..., "public": ...}}
    ...
    def generate_symmetric_key(self, key_name: str, algorithm: Optional[EncryptionAlgorithm] = None) -> bytes:
        """
    ...
    def set_symmetric_key(self, key_name: str, key: bytes) -> None:
        ...
        with self.lock:
            ...
            try:
                ...
                # Not a valid Fernet key, which is fine
                pass
        ...
        with self.lock:
            ...
            for key_name in list(self.key_pairs.keys()):
                if key_name.endswith("_old"):
                    ...
                    self.key_pairs[key_name] = {"private": private_pem, "public": public_pem}
            
            self.last_rotation_time = current_time
            logger.info(f"Rotated encryption keys: {len(self.symmetric_keys)} symmetric keys, {len(self.key_pairs)} key pairs")
    ...
    def encrypt_data(
        ...
        try:
            ...
            if algo == EncryptionAlgorithm.FERNET:
                ...
                encrypted = meta_len.to_bytes(2, byteorder='big') + meta_bytes + iv + tag + encrypted_data
                ...
                if len(data_bytes) > 190:  # Adjust based on key size and padding
                    raise ValueError("Data too large for RSA encryption. Use symmetric encryption for large data.")
