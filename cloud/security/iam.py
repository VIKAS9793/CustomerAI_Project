    ...
    def _start_credential_refresh_thread(self):
        ...
        if not self._refresh_thread_running:
            ...
            refresh_thread = threading.Thread(
                target=self._credential_refresh_task,
                daemon=True
            )
                ...
                for name, credential in list(creds.items()):
                    ...
                    if credential.expiry - now <= self.credential_refresh_threshold:
                        try:
                            ...
                            logger.warning(f"Failed to refresh credential {provider}/{name}: {str(e)}")
    ...
    def _refresh_aws_credential(self, credential: Credential, name: str) -> Optional[Credential]:
        ...
        if credential.type == "iam_role":
            try:
                ...
                if not role_arn:
                    logger.warning("Cannot refresh AWS IAM role credential: role_arn not found in metadata")
                    return None
        ...
        if credential.type == "service_principal":
            try:
                ...
                if not all([tenant_id, client_id, client_secret]):
                    logger.warning("Cannot refresh Azure service principal: missing required metadata")
                    return None
