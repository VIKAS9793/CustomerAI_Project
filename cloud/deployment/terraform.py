...
class TerraformDeployer:
    ...
    def plan(
        self, out_file: Optional[str] = None, detailed_exitcode: bool = False
    ) -> Union[str, int]:
        """
        ...
        with self.lock:
            ...
            try:
                process = self._run_command(
                    TerraformCommand.PLAN, args, check=not detailed_exitcode
                )
                self.last_output = process.stdout
                self.status = DeploymentStatus.COMPLETED
    ...
    def apply(self, plan_file: Optional[str] = None, target: Optional[List[str]] = None) -> str:
        """
    ...
    def destroy(self, target: Optional[List[str]] = None) -> str:
        """
...
class CloudTemplateManager:
    ...
    def __init__(self, template_directory: str, output_directory: str):
        """
...
def deploy_infrastructure(
    ...
    template_manager = CloudTemplateManager(
        template_directory=template_directory, output_directory=output_directory
    )

    # Generate template
    template_path = template_manager.generate_template(
        provider=provider, template_name=template_name, variables=variables
    )

    # Create Terraform deployer
    deployer = TerraformDeployer(working_directory=template_path, auto_approve=auto_approve)
...
def destroy_infrastructure(deployment_path: str, auto_approve: bool = False) -> None:
    """
    ...
    """
    # Create Terraform deployer
    deployer = TerraformDeployer(working_directory=deployment_path, auto_approve=auto_approve)
...
def get_deployment_outputs(deployment_path: str) -> Dict[str, Any]:
    """
    ...
    """
    # Create Terraform deployer
    deployer = TerraformDeployer(working_directory=deployment_path)
...
def create_gcp_storage_backend(
    bucket: str, prefix: str = "terraform/state", credentials: Optional[str] = None
) -> Dict[str, str]:
    """
    ...
    """
    # Create backend configuration
    backend_config = {"bucket": bucket, "prefix": prefix}
