...
class BaseLLMClient(ABC):
    ...
    @abstractmethod
    async def classify_text(self, text: str, categories: List[str], **kwargs) -> Dict[str, float]:
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if attempt < max_retries:
                    ...
                    logger.warning(
                        f"Retry {attempt+1}/{max_retries} after error: {str(e)}. Sleeping {sleep_time}s"
                    )
...
class OpenAIClient(BaseLLMClient):
    ...
    async def get_embeddings(self, texts: List[str], **kwargs) -> List[List[float]]:
        ...
        response = await self.client.embeddings.create(
            model=model, input=texts, **self.config.additional_params
        )
...
class GoogleAIClient(BaseLLMClient):
    ...
    def _initialize_client(self) -> Any:
        ...
        try:
            ...
            raise ImportError(
                "Google Generative AI package not installed. Run 'pip install google-generativeai>=0.3.0'"
            )
...
class LLMClientFactory:
    ...
    def create_financial_client(
        ...
        config = LLMConfig(
            ...
            additional_params={"user": "financial-services", "safe_mode": True},
        )
