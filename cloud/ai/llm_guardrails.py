...
class CommonSensitivePatterns:
    ...
    SQL_INJECTION = r"\b(?:SELECT|INSERT|UPDATE|DELETE|DROP|ALTER|UNION|JOIN|WHERE)\b.*\b(?:FROM|INTO|TABLE)\b"
...
class LLMGuardrails:
    ...
    def filter_input(self, content: str, context: Optional[Dict[str, Any]] = None) -> GuardrailResult:
        """Apply input guardrails to user content"""
        return self._apply_guardrails(content, GuardrailDirection.INPUT, context)
    
    def filter_output(self, content: str, context: Optional[Dict[str, Any]] = None) -> GuardrailResult:
    ...
    def _apply_guardrails(self, content: str, direction: GuardrailDirection, 
        ...
        for name, check in self.checks.items():
            ...
            if (direction == GuardrailDirection.INPUT and check.category in 
                [GuardrailCategory.SAFETY, GuardrailCategory.SECURITY, GuardrailCategory.PRIVACY]):
                pass  # Apply check to input
            elif (direction == GuardrailDirection.OUTPUT and check.category in 
                  [GuardrailCategory.QUALITY, GuardrailCategory.ETHICS, GuardrailCategory.COMPLIANCE]):
    ...
    def _apply_regex_check(self, content: str, check: GuardrailCheck, 
                           context: Dict[str, Any]) -> List[GuardrailViolation]:
        ...
        flags = re.IGNORECASE if check.rule_config.get("case_sensitive", True) == False else 0
        ...
        for pattern_name, pattern in patterns.items():
            for match in re.finditer(pattern, content, flags):
                ...
                if check.category == GuardrailCategory.SECURITY:
                    high_risk_terms = ["ignore all previous instructions", "ignore your instructions", 
                                     "new persona", "system prompt", "bypass restrictions"]
    ...
    def _apply_semantic_check(self, content: str, check: GuardrailCheck, 
                              context: Dict[str, Any]) -> List[GuardrailViolation]:
        ...
        category_keywords = {
            ContentCategories.HATE: [
                "hate", "hating", "despise", "loathe", "detest", 
                "racist", "racism", "bigot", "bigoted", "prejudice"
            ],
            ContentCategories.VIOLENCE: [
                "kill", "killing", "murder", "shoot", "shooting", "attack", 
                "attacking", "beat", "beating", "assault", "violent"
            ],
            ContentCategories.SEXUAL: [
                "explicit", "pornographic", "sex", "sexual", "nude", 
                "naked", "porn", "nsfw", "18+"
            ],
            ContentCategories.SELF_HARM: [
                "suicide", "suicidal", "self-harm", "cut myself", 
                "kill myself", "end my life", "hurt myself"
            ],
            ContentCategories.HARASSMENT: [
                "harass", "harassment", "bully", "bullying", "stalk", 
                "stalking", "threaten", "threatening"
            ],
            ContentCategories.DISCRIMINATION: [
                "discriminate", "discrimination", "sexist", "sexism", 
                "misogyny", "misogynist", "homophobic", "homophobia", 
                "transphobic", "transphobia", "ableist", "ableism"
            ],
            ContentCategories.ILLEGAL_ACTIVITY: [
                "illegal", "crime", "criminal", "drugs", "drug dealing", 
                "hack", "hacking", "steal", "stealing", "fraud", "scam"
            ]
        }
    ...
    def _apply_external_api_check(self, content: str, check: GuardrailCheck, 
                                  context: Dict[str, Any]) -> List[GuardrailViolation]:
    ...
    def _apply_custom_check(self, content: str, check: GuardrailCheck, 
                            context: Dict[str, Any]) -> List[GuardrailViolation]:
    ...
    def detect_uncertainty(self, content: str, check: GuardrailCheck, 
                         context: Dict[str, Any]) -> List[GuardrailViolation]:
    ...
    for test_input in test_inputs:
        ...
        if result.violations:
            ...
            for v in result.violations:
                print(f"- {v.category}: {v.description} (Risk: {v.risk_level}, Confidence: {v.confidence:.2f})")
