...
class LLMManager:
    ...
    def _load_config(self, config_path: str) -> None:
        ...
        try:
            ...
            for client_id, client_config in config_data.get("clients", {}).items():
                ...
                # Create LLM config
                client_capabilities_raw = client_config.get("capabilities", ["text_generation"])
                llm_capabilities = [LLMCapability(cap) for cap in client_capabilities_raw]
                llm_config = LLMConfig(
                    provider=provider,
                    api_key=client_config.get("api_key"),
                    api_endpoint=client_config.get("api_endpoint"),
                    model_name=client_config.get("model_name"),
                    compliance_level=LLMComplianceLevel(
                        client_config.get("compliance_level", "standard")
                    ),
    ...
    def register_client(self, client_id: str, config: Union[LLMConfig, Dict[str, Any]]) -> None:
        ...
        if isinstance(config, dict):
            ...
            # Temp variable for capabilities
            raw_capabilities = config.get("capabilities", ["text_generation"])
            processed_capabilities = [LLMCapability(cap) for cap in raw_capabilities]
    ...
    async def generate_text(
        self, prompt: str, client_id: Optional[str] = None, **kwargs
    ) -> Dict[str, Any]:
        """
    ...
    async def get_embeddings(
        self, texts: List[str], client_id: Optional[str] = None, **kwargs
    ) -> List[List[float]]:
        """
