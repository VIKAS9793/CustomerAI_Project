...
from cloud.utils.metrics import MetricType, get_metrics_manager
...
class PrometheusExporter:
    ...
    def _get_metric_name(
        self, provider: str, service: str, operation: str, metric_type: str
    ) -> str:
        """
    ...
    def _get_or_create_metric(
        ...
        with self.lock:
            ...
            if category == MetricCategory.COUNTER:
                metric = Counter(metric_name, description, labels or [], registry=self.registry)
            elif category == MetricCategory.GAUGE:
                metric = Gauge(metric_name, description, labels or [], registry=self.registry)
            elif category == MetricCategory.HISTOGRAM:
                metric = Histogram(metric_name, description, labels or [], registry=self.registry)
            elif category == MetricCategory.SUMMARY:
                metric = Summary(metric_name, description, labels or [], registry=self.registry)
            else:
                ...
                for service in metrics_data[provider]:
                    for operation in metrics_data[provider][service]:
                        for metric_type_str in metrics_data[provider][service][operation]:
                            metric_data = metrics_data[provider][service][operation][
                                metric_type_str
                            ]
    ...
    def start_export_thread(self) -> None:
        ...
        if not self._export_thread_running and self.enabled and PROMETHEUS_CLIENT_AVAILABLE:
            ...
            export_thread = threading.Thread(target=self._export_task, daemon=True)
            export_thread.start()
            self._export_thread_running = True

            logger.info(
                f"Started Prometheus metrics export thread (interval: {self.export_interval}s)"
            )
...
def configure_prometheus_export(
    enabled: bool = True, export_interval: int = 15, metric_prefix: str = "cloud"
) -> None:
    """
...
def with_prometheus_metrics(
    name: Optional[str] = None, labels: Optional[Dict[str, str]] = None
