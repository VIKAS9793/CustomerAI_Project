...
def api_request(endpoint, method="GET", data=None, params=None):
    headers = {"Authorization": f"Bearer {st.session_state.token}"} if st.session_state.token != "demo_token" else {}
        ...
        if endpoint == "analytics/summary":
            return {
                "data": {
                    ...
                    "sentiment_distribution": {"positive": 0.65, "negative": 0.18, "neutral": 0.17},
...
def dashboard_page():
    ...
    if response:
        ...
        fig = alt.Chart(issues_data).mark_bar().encode(
            x=alt.X("count:Q", title="Number of Conversations"),
            y=alt.Y("issue:N", sort="-x", title="Issue Category"),
            color=alt.Color("sentiment:Q", scale=alt.Scale(scheme="redblue"), title="Sentiment Score")
        ...
        if text:
            with st.spinner("Analyzing..."):
                ...
                if response:
                    ...
                    col2.metric("Satisfaction Score", f"{data['analysis']['satisfaction_score']}/10")
                    
                    # Sentiment scores
                    st.subheader("Sentiment Breakdown")
                    scores_data = pd.DataFrame({
                        "Sentiment": ["Positive", "Negative", "Neutral"],
                        "Score": [data["positive"], data["negative"], data["neutral"]]
                    })
