...
class FairnessDashboard:
    ...
    def load_results(self, results_dict: Dict = None, results_file: str = None) -> None:
        ...
        if results_dict:
            ...
            try:
                ...
                st.error("The results file is too large to load into memory. Please use a smaller file.")
                self.results = None
        else:
    ...
    def _display_summary(self) -> None:
        ...
        st.write(f"**Attributes Analyzed:** {', '.join(dataset_info.get('attributes_analyzed', []))}")
            ...
            for outcome, outcome_data in attr_data.get('outcome_metrics', {}).items():
                for metric, value in outcome_data.get('metrics', {}).items():
                    ...
                    if processed_count >= max_items:
                        st.warning(f"Only showing first {max_items} metrics. Use filters to narrow down results.")
                        break
                    ...
                    if groups in outcome_data.get('statistical_significance', {}):
                        ...
                        is_significant = outcome_data['statistical_significance'][groups]['is_significant']
    ...
    def _display_detailed_metrics(self) -> None:
            ...
            if not filtered_df.empty:
                ...
                if 'disparate_impact' in selected_metric:
                    fig = px.bar(
                        ...
                        color='outcome',
                        barmode='group',
                        labels={'value': 'Disparate Impact Ratio', 'attribute': 'Protected Attribute'},
                        hover_data=['group1', 'group2', 'significance', 'p_value']
                    )
                    
                    # Add threshold lines
                    threshold = self.results.get('summary', {}).get('threshold', 0.8)
                    fig.add_shape(type="line", x0=-0.5, x1=len(filtered_df['attribute'].unique())-0.5, 
                                 y0=threshold, y1=threshold, line=dict(color="red", width=2, dash="dash"))
                    fig.add_shape(type="line", x0=-0.5, x1=len(filtered_df['attribute'].unique())-0.5, 
                                 y0=1.0, y1=1.0, line=dict(color="green", width=2))
                    fig.add_shape(type="line", x0=-0.5, x1=len(filtered_df['attribute'].unique())-0.5, 
                                 y0=1/threshold, y1=1/threshold, line=dict(color="red", width=2, dash="dash"))
                    ...
                    **Interpretation:**
                    ...
                    fig = px.bar(
                        ...
                        color='outcome',
                        barmode='group',
                        labels={'value': 'Statistical Parity Difference', 'attribute': 'Protected Attribute'},
                        hover_data=['group1', 'group2', 'significance', 'p_value']
                    )
                    
                    # Add zero line
                    fig.add_shape(type="line", x0=-0.5, x1=len(filtered_df['attribute'].unique())-0.5, 
                                 y0=0, y1=0, line=dict(color="green", width=2))
                    ...
                    **Interpretation:**
                    ...
                    fig = px.bar(
                        ...
                        color='outcome',
                        barmode='group',
                        labels={'value': selected_metric, 'attribute': 'Protected Attribute'},
                        hover_data=['group1', 'group2', 'significance', 'p_value']
                    )
                    st.plotly_chart(fig, use_container_width=True)
            else:
        ...
        with tab2:
            ...
            if not attr_df.empty:
                ...
                attr_data = attribute_results.get(selected_attr, {})
