...
class BiasDetector:
    ...
    def detect_outcome_bias(
        ...
        for attribute in valid_attributes:
            ...
            results['attribute_results'][attribute] = {
                ...
                'distribution': {str(val): (data[attribute] == val).mean() 
            ...
            for outcome in outcome_columns:
                ...
                outcome_results = self._analyze_outcome(
                    data, attribute, outcome, pos_val
                )
                
                results['attribute_results'][attribute]['outcome_metrics'][outcome] = outcome_results
    ...
    def _analyze_outcome(
        ...
        if len(metrics_by_group) >= 2:
            ...
            reference_group = max(metrics_by_group.items(), 
            ...
            for group_name, group_metrics in metrics_by_group.items():
                ...
                if 'disparate_impact' in self.metrics:
                    if reference_metrics['positive_rate'] > 0:
                        disparate_impact = group_metrics['positive_rate'] / reference_metrics['positive_rate']
                    else:
                        disparate_impact = float('inf') if group_metrics['positive_rate'] > 0 else 1.0
                    
                    result['metrics'][f'disparate_impact_{group_name}_vs_{reference_name}'] = disparate_impact
                
                # Calculate statistical parity difference
                if 'statistical_parity' in self.metrics:
                    stat_parity = group_metrics['positive_rate'] - reference_metrics['positive_rate']
                    result['metrics'][f'statistical_parity_{group_name}_vs_{reference_name}'] = stat_parity
                
                # Calculate statistical significance
                p_value = self._calculate_significance(
                    group_metrics['positive_rate'], reference_metrics['positive_rate'],
                    group_metrics['sample_size'], reference_metrics['sample_size']
                )
    ...
    def _calculate_significance(
        ...
        try:
            ...
            z_score = (rate1 - rate2) / np.sqrt((rate1 * (1 - rate1) / n1) + (rate2 * (1 - rate2) / n2))
    ...
    def _generate_summary(self, attribute_results: Dict) -> Dict:
            ...
            for outcome_name, outcome_data in attr_data.get('outcome_metrics', {}).items():
                for metric_name, metric_value in outcome_data.get('metrics', {}).items():
                    ...
                    if 'disparate_impact' in metric_name:
                        ...
                        is_fair = (self.fairness_threshold <= metric_value <= (1/self.fairness_threshold))
        ...
        if summary['bias_detected']:
            summary['recommendations'] = self._generate_recommendations(summary['significant_findings'])
    ...
    def _generate_recommendations(self, findings: List[Dict]) -> List[str]:
        ...
        for attr, attr_findings_list in attr_findings.items():
            ...
            disparate_findings = [f for f in attr_findings_list if 'disparate_impact' in f['metric']]
