...
class ReviewQueue:
    ...
    def __init__(
        self,
        name: str,
        categories: List[ReviewCategory] = None,
        max_size: int = 1000,
        ...
        with self.lock:
            ...
            for item in self.items.values():
                ...
                if ReviewerRole.ADMINISTRATOR in reviewer.roles:
                    ...
                    ReviewCategory.FINANCIAL_ADVICE,
                    ReviewCategory.COMPLIANCE_CHECK,
                ]:
                    matching_items.append(item)
                elif (
                    ReviewerRole.COMPLIANCE in reviewer.roles
                    and item.category == ReviewCategory.COMPLIANCE_CHECK
                ):
                    matching_items.append(item)
                elif ReviewerRole.SUBJECT_MATTER_EXPERT in reviewer.roles and any(
                    spec in item.metadata.get("specialization_tags", [])
                    for spec in reviewer.specializations
                ):
                    matching_items.append(item)
                elif ReviewerRole.GENERAL in reviewer.roles and item.category in [
                    ReviewCategory.CUSTOMER_RESPONSE,
                    ReviewCategory.SENTIMENT_ANALYSIS,
            ...
            if os.getenv("SLACK_API_TOKEN") and os.getenv("SLACK_CHANNEL_ID"):
                ...
                blocks = [
                    ...
                    },
                    {
                        "type": "section",
                        "fields": [
                            {"type": "mrkdwn", "text": f"*ID:* {item.item_id}"},
                            {
                                "type": "mrkdwn",
    ...
    def _record_review_decision(self, item: ReviewItem) -> None:
        ...
        try:
            ...
            feedback_file = os.path.join(
                feedback_dir,
                f"{datetime.DateProvider.get_instance().now().strftime('%Y%m%d')}.jsonl",
            )
...
class ReviewManager:
    ...
    def create_queue(
        self, name: str, categories: List[ReviewCategory] = None, max_size: int = 1000
    ) -> ReviewQueue:
            ...
            stats["reviewer_loads"].append(
                {
                    ...
                    "load_percentage": (
                        round(
                            (reviewer.current_daily_reviews / reviewer.max_daily_reviews) * 100,
                            2,
                        )
                        if reviewer.max_daily_reviews > 0
                        else 0
                    ),
                }
            )
        ...
        while self.is_running:
            ...
            for queue in self.queues.values():
                with queue.lock:
                    for item in queue.items.values():
                        if (
                            item.status in [ReviewStatus.PENDING, ReviewStatus.IN_PROGRESS]
                            and item.due_by
                            and now > item.due_by
                        ):
                            # Item has expired
                            ...
                            if item.priority in [
                                ...
                                logger.warning(
                                    f"High priority review item {item.item_id} has expired"
                                )

                                # If integration enabled, send escalation
                                if os.getenv("SLACK_API_TOKEN") and os.getenv(
