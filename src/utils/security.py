...
if not JWT_SECRET_KEY:
    logger.warning(
        "JWT_SECRET_KEY not set! Using random value - this will invalidate all tokens on restart"
    )
    JWT_SECRET_KEY = secrets.token_hex(32)

if not ENCRYPTION_KEY:
    logger.warning(
        "ENCRYPTION_KEY not set! Using random value - this will break existing encrypted data"
    )
...
class DeviceSecurityValidator:
    ...
    def is_device_secure(self, request_data: Dict[str, Any]) -> Tuple[bool, str]:
        ...
        if self._has_unlocked_bootloader(device_fingerprint):
            self.logger.warning(
                f"Rejected request from device with unlocked bootloader: {device_id}"
            )
    ...
    def _is_device_rooted(self, fingerprint: Dict[str, Any]) -> bool:
        ...
        if "android" in platform:
            ...
            jailbreak_apps = ["Cydia", "Sileo", "Zebra", "Installer"]
            installed_apps = fingerprint.get("installed_apps", [])
