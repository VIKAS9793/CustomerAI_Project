    ...
    def validate_metrics(cls, metrics):
        ...
        for group, group_metrics in metrics.items():
            for metric_name, value in group_metrics.items():
                ...
                if value < 0 and metric_name not in [
                    ModelPerformanceMetric.RMSE,
                    ModelPerformanceMetric.MAE,
                ]:
...
class ModelCard(BaseModel):
    ...
    document_id: str = Field(
        default_factory=lambda: f"model-card-{DateProvider.get_instance().now().strftime('%Y%m%d-%H%M%S')}"
    )
    ...
    def to_html(self) -> str:
        ...
        if self.quantitative_analysis.performance_by_group:
            ...
            for (
                group,
                group_metrics,
            ) in self.quantitative_analysis.performance_by_group.items():
        ...
        if self.considerations.fairness_considerations:
            consideration_types.append(
                ("Fairness Considerations", self.considerations.fairness_considerations)
            )
        ...
        for dataset in self.datasets:
            dataset_link = (
                f"<a href='{dataset.link}'>{dataset.name}</a>" if dataset.link else dataset.name
            )
            html += f"""
                <div class="consideration">
                    <h3>{dataset_link}</h3>
    ...
    return ModelCard(
        ...
        quantitative_analysis=ModelCardQuantitativeAnalysis(
            ...
            performance_by_group={
                "Financial News": {"accuracy": 0.912, "f1": 0.896},
                "Customer Feedback": {"accuracy": 0.874, "f1": 0.857},
                "Earnings Calls": {"accuracy": 0.889, "f1": 0.871},
            },
            decision_thresholds={"positive": 0.6, "negative": 0.6, "neutral": 0.5},
        ),
        considerations=ModelCardConsiderations(
            users=[
        ...
        datasets=[
            ModelCardDataset(
                ...
                data_splits={"train": 3000, "validation": 500, "test": 500},
                data_format="Text with sentiment labels",
                known_biases=[
                    "Over-representation of Western financial markets",
            ...
            ModelCardDataset(
                ...
                data_splits={"train": 25000, "validation": 5000, "test": 5000},
...
class ModelCardRegistry:
        ...
        return [
            {
                ...
                "description": (
                    card.model_description[:100] + "..."
                    if len(card.model_description) > 100
                    else card.model_description
                ),
    ...
    def get_model_cards_by_type(self, model_type: str) -> List[ModelCard]:
        ...
        return [card for card in self.model_cards.values() if card.model_details.type == model_type]
    ...
    def get_model_cards_by_compliance(self, compliance_spec: str) -> List[ModelCard]:
        ...
        return [
            card
            for card in self.model_cards.values()
            if card.model_details.compliance_specifications
            and compliance_spec in card.model_details.compliance_specifications
        ]
